...
async def store_deletion_request(db, user_id: str, field: Optional[str] = None):
    request = {
        "user_id": user_id,
...
# ===================== KEYWORD FALLBACK =====================
async def keyword_fallback(user_id: str, message: str, db, logger_svc) -> dict:
    ...
    if "delete all my data" in msg_lower or "delete all data" in msg_lower:
        await store_deletion_request(db, user_id, "all")
    ...
    for field, keywords in fields.items():
        for kw in keywords:
            if f"delete my {kw}" in msg_lower or f"remove my {kw}" in msg_lower:
                await store_deletion_request(db, user_id, field)
...
# ===================== MAIN PROCESSOR =====================
async def process_chat(user_id: str, message: str, db, logger_svc, deletion_svc) -> dict:
    ...
    try:
        ...
        fallback = await keyword_fallback(user_id, message, db, logger_svc)
        action = fallback["action"]
        field = fallback.get("field")
        ai_reply = fallback["response"]
    ...
    if action == "delete_field" and field:
        ...
        # Optionally fetch user data via deletion_svc.get_user_data(user_id)
...
async def chat_endpoint(req: ChatRequest, db=Depends(get_db)):
    try:
        ...
        result = await process_chat(req.user_id, req.message, db, logger_svc, deletion_svc)
        return ChatResponse(
